...
STANDARD_MELEE_RANGE = 1
HALBERD_MELEE_RANGE = 2

HALBERD_WEAPON_IDS = {
    3190, 3192, 3194, 3196, 20431, 25985,
}
...
PRAYER_LOGIC = {
    "piety": {
        "max": ["piety"],
        "zerker": ["ultimate_strength", "incredible_reflexes"],
        "pure": ["ultimate_strength", "incredible_reflexes"]
    },
    "rigour": {
        "max": ["rigour"],
        "zerker": ["eagle_eye"],
...
DARK_BOW_ID = 20408
DRAGON_ARROW_ID = 20389
ELDER_MAUL_ID = 21205
DHAROKS_GREATAXE_ID = 25516
VERACS_FLAIL_ID = 27189
GMAUL_ID = 20557
MAGES_BOOK_ID = 6889
SPIRIT_SHIELD_ID = 23599
UNHOLY_BOOK_ID = 27191

WAIT_600_BASE = 0.6
WAIT_600_OFFSET = 0.045
...
if not os.path.isfile(MANUAL_COORDS_PATH):
    print(f"ERROR: manual_coords_mapping.json not found at {MANUAL_COORDS_PATH}")
    sys.exit(1)
if not os.path.isfile(SWITCHER_HOTKEYS_PATH):
    print(f"ERROR: switcher_hotkeys.json not found at {SWITCHER_HOTKEYS_PATH}")
    sys.exit(1)

with open(MANUAL_COORDS_PATH, "r") as f:
    COORDS_MAP = json.load(f)
with open(SWITCHER_HOTKEYS_PATH, "r") as f:
    SWITCHER_HOTKEYS = json.load(f)

locked_bracket = None
locked_bracket_lock = threading.Lock()
last_f11_time = 0
last_f11_time_lock = threading.Lock()

# Action lock and queue for seamless action sequencing
action_lock = threading.Lock()
action_queue = deque()
queue_event = threading.Event()

# --- Prayer switch fix: track last prayer/tick attempted ---
...
def is_in_melee_range(player_pos, opponent_pos, opponent_weapon_id):
    ...
    if is_halberd_weapon(opponent_weapon_id):
        return distance <= HALBERD_MELEE_RANGE
    else:
...
def fast_move_and_click(x, y, delay=None, stop_event=None, jitter=BEZIER_JITTER, context=""):
    ...
    rx = x + random_offset()
    ry = y + random_offset()
    move_mouse_bezier(rx, ry, total_duration=delay, steps=BEZIER_STEPS, jitter=jitter, stop_event=stop_event,
                      context=context)
    if stop_event and stop_event.is_set():
...
def switch_gear(wanted_items, inventory, equipment, stop_event=None, skip_defender=False):
    ...
    # Query current tab from game_state after switching to inventory
    ...
    def click_inventory_slot(x, y, context=""):
        if (current_tab or "").lower() == "inventory":
            ...
            released_lshift = False
            released_rshift = False
            try:
                if lshift_was_down:
...
def get_current_protection_prayer():
    ...
    protection_prayers = ["protect_from_magic", "protect_from_missiles", "protect_from_melee"]
    for prot in protection_prayers:
...
def switch_prayers(prayer_names, prayers, stop_event=None, protection_prayer=None):
    ...
    # Only allow valid protection prayers
    ...
    try:
        ...
        if need_protection_switch:
            ...
            if coords:
                ...
                fast_move_and_click(x, y, delay=rand_prayer_click_delay(), stop_event=stop_event,
                                    context=f"PRIORITY_protection_{protection_prayer}")
                debug(f"[Prayer Switch] EXECUTED protection switch to {protection_prayer}", always=True)
        ...
        for pr in prayers_to_activate:
            ...
            if coords:
                ultra_accurate_sleep(rand_gear_sleep(), stop_event=stop_event)
                fast_move_and_click(coords['x'], coords['y'], delay=rand_prayer_click_delay(), stop_event=stop_event,
                                    context=f"offensive_{pr}")
...
auto_protect_lock = threading.Lock()
hotkey_sequence_active = False
hotkey_sequence_lock = threading.Lock()
prayer_tab_protection_active = False
...
def get_recommended_protection_prayer():
    ...
    state = game_state.get()
    prayer_to_activate = state.get("recommended_protection_prayer")
    allowed_prayers = {"protect_from_magic", "protect_from_missiles", "protect_from_melee", "augury", "rigour"}
    if not prayer_to_activate or prayer_to_activate not in allowed_prayers:
        return None

    current_protection = get_current_protection_prayer()
    if current_protection == prayer_to_activate:
        debug(f"[Click Block] Recommended prayer '{prayer_to_activate}' already active, no switch needed", always=True)
        return None

    opponent = state.get("opponent")
    if opponent:
        player_pos = state.get("player_world_position")
        opponent_pos = opponent.get("world_position")
        if player_pos and opponent_pos:
            opponent_weapon_id = -1
            in_melee_range = is_in_melee_range(player_pos, opponent_pos, opponent_weapon_id)
            if not in_melee_range and "melee" in prayer_to_activate.lower():
                debug(f"[Click Block] Opponent out of melee range, skipping melee protection", always=True)
                return None
...
def standalone_protection_thread():
    ...
    while True:
        time.sleep(0.05)  # update prayer checks more frequently
    ...
    if pressed and button == pynput_mouse.Button.left and current_tab == "prayer" and is_auto_protect_enabled():
        debug("[INPUT BLOCK] Blocked user click on prayer tab due to auto-pray", always=True)
        return
    if pressed and button == pynput_mouse.Button.left and current_tab == "prayer" and is_auto_protect_enabled():
        debug("[INPUT BLOCK] Blocked user click on prayer tab due to auto-pray", always=True)
        return
...
def run_switch(hotkey, stop_event):
    ...
    def do_switch():
        try:
            ...
            has_gmaul = any(slot.get("item_id") == GMAUL_ID for slot in inventory.values()) \
                        or any(slot.get("item_id") == GMAUL_ID for slot in equipment.values())
            ...
            if hotkey == "F11" and has_gmaul:
                ...
                if prayers_for_bracket or protection_prayer:
                    switch_prayers(prayers_for_bracket or [], prayers, stop_event=stop_event,
